...
class MockProjectIo(ProjectIoInterface):
    ...
    def load_scheme(self, file_name: StrOrPath, **kwargs: Any) -> Scheme:
        """This docstring is just for help testing of 'load_scheme'."""
        mock_obj = MockFileLoadable()
        mock_obj.func_args = {"file_name": file_name, **kwargs}
        return mock_obj  # type:ignore[return-value]
    ...
    for format_name, plugin_class in [("dummy", Dummy), ("dummy2", Dummy2), ("dummy3", Dummy2)]:
        ...
        assert isinstance(
            __PluginRegistry.project_io[
                f"tests.plugin_system.test_project_io_registration.{plugin_class.__name__}_{format_name}"
            ],
            plugin_class,
        )
...
def test_load_functions(tmp_path: Path, load_function: Callable[..., Any]):
    ...
    assert Path(result.source_path) == Path(file_path)
...
def test_write_functions(
    ...
    if update_source_path is True:
        assert Path(mock_obj.source_path) == file_path
    else:
...
@pytest.mark.parametrize(
    "function",
    [save_parameters, save_scheme, save_result],
)
@pytest.mark.usefixtures("mocked_registry")
...
def test_get_project_io_method():
    ...
    result = get_project_io_method("mock", "load_scheme")

    assert result.__code__ == io.load_scheme.__code__
...
def test_show_project_io_method_help(capsys: CaptureFixture):
    ...
    help(plugin.load_scheme)
    original_help, _ = capsys.readouterr()

    show_project_io_method_help(format_name="mock", method_name="load_scheme")
    result, _ = capsys.readouterr()

    assert "This docstring is just for help testing of 'load_scheme'." in result
...
def test_project_io_plugin_table():
    ...
    expected = dedent(
        """\
        |  __Format name__  |  __load_parameters__  |  __save_parameters__  |  __load_scheme__  |  __save_scheme__  |  __load_result__  |  __save_result__  |
        |-------------------|-----------------------|-----------------------|-------------------|-------------------|-------------------|-------------------|
        |       `foo`       |           /           |           /           |         /         |         /         |         /         |         /         |
        |      `mock`       |           *           |           *           |         *         |         *         |         *         |         *         |
        """  # noqa: E501
...
def test_project_io_plugin_table_full():
    ...
    expected = dedent(
        """\
        |                 __Format name__                  |  __load_parameters__  |  __save_parameters__  |  __load_scheme__  |  __save_scheme__  |  __load_result__  |  __save_result__  |                            __Plugin name__                            |
        |--------------------------------------------------|-----------------------|-----------------------|-------------------|-------------------|-------------------|-------------------|-----------------------------------------------------------------------|
        |                      `foo`                       |           /           |           /           |         /         |         /         |         /         |         /         |            `glotaran.io.interface.ProjectIoInterface_foo`             |
        |                      `mock`                      |           *           |           *           |         *         |         *         |         *         |         *         | `tests.plugin_system.test_project_io_registration.MockProjectIo_mock` |
