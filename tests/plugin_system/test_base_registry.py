...
def get_mock_plugin_function(plugin_registry_key: str):
    if plugin_registry_key == "base":
        return MockPlugin
    if plugin_registry_key == "sub_class_partial":
        return MockPluginSubclassPartial
    if plugin_registry_key == "sub_class_full":
        return MockPluginSubclassFull
    if plugin_registry_key == "sub_class_str":
        return MockPluginSubclassStr
    msg = f"No mock plugin with name {plugin_registry_key!r}"
...
def test_PluginOverwriteWarning():  # noqa: N802
...
def test_add_plugin_to_register_naming_error():
    ...
    with pytest.raises(
        ...
        add_plugin_to_registry("bad.name", MockPlugin, mock_registry_data_io, "set_plugin")
...
def test_methods_differ_from_baseclass_table_plugin_names():
    ...
    assert list(result) == [
        ["`base`", False, "`tests.plugin_system.test_base_registry.MockPlugin`"]
    ]
