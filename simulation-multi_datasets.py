...
def fillK(kmat):
    reskmat = kmat.copy()
    idx = np.diag_indices(reskmat.shape[0])
    reskmat[idx] = - reskmat.sum(axis=0)
    return reskmat
...
def main():
    ...
    # simulation code that was implemented below.
    #    
    ...
    #print(kmatrix1)
    #kmatrix11 = fillK(None, None, kmatrix1, True, None, None)# call fillK
    (A1, k1) = fullKF(kmatrix1, jvec1)# call fullKF
    # get k and A
    
    print(A1, k1)
    
    # megacomplex 2 (m2)
    location2 = np.asarray([14727.541, 14598.540, 14388.490, 0])
    delta2 = np.asarray([350,600,400, 0])
