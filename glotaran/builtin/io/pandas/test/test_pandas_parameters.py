...
def test_save_parameters_csv_with_different_parameter_types(
    tmp_path: Path,
):
    """load yaml parameter file which has different parameter types,
    then save parameters in csv file.
    load csv file and compare with yaml parameter file"""
    parameters_yaml_diff_param = load_parameters(PATH_YAML_DIFF_PARAM)
    parameter_path = tmp_path / "testparameters.csv"
    save_parameters(
        file_name=parameter_path, format_name="csv", parameters=parameters_yaml_diff_param
    )
    parameters_saved_and_reloaded = load_parameters(parameter_path)
    parameters_csv = load_parameters(PATH_CSV)
    assert parameters_yaml_diff_param == parameters_saved_and_reloaded
...
def test_load_parameters_csv_with_different_parameter_types():
    """load parameter file as yaml and csv and compare them
    also load parameter file with different parameter types and compare with other"""
    parameters_csv = load_parameters(PATH_CSV)
    parameters_yaml = load_parameters(PATH_YAML)
    parameters_yaml_diff_param = load_parameters(PATH_YAML_DIFF_PARAM)
