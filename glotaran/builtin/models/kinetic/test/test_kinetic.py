...
from glotaran.builtin.models.kinetic import Kinetic
...
def test_a_matrix_general(model):
    ...
    kinetic = Kinetic(label="", rates=model.rates)
    assert kinetic.compartments == model.compartments
...
def test_a_matrix_sequential():
    ...
    kinetic = Kinetic(label="", rates=rates)
...
def test_combine_matrices():
    ...
    mat1 = Kinetic(label="A", rates=matrix1)
    matrix2 = {
        ("s2", "s2"): 3,
        ("s3", "s3"): 4,
    }
    mat2 = Kinetic(label="B", rates=matrix2)
